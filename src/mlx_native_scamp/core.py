...
def _parse_common_kwargs(
    ...
    valid_keys = {"verbose", "precision", "pearson", "gpus", "threads", "allow_trivial_match"}
    if allow_threshold:
    ...
    if not is_ab_join and "allow_trivial_match" in kwargs:
        raise ValueError(
            "allow_trivial_match is only valid for ab-joins; self-joins always exclude trivial matches."
        )
    ...
    params = {
        ...
        "allow_trivial_match": bool(kwargs.get("allow_trivial_match", True)),
    }
    if allow_matrix:
...
def _iterate_blocks(prepared_a: PreparedSeries, prepared_b: PreparedSeries, exclusion: int, block_rows: int):
...
def _best_match_profile(
    ...
    for row_start, _, _, block in _iterate_blocks(prepared_a, prepared_b, exclusion, block_rows=BLOCK_ROWS):
...
def _sum_threshold_profile(
    ...
    for _, _, _, block in _iterate_blocks(prepared_a, prepared_b, exclusion, block_rows=BLOCK_ROWS):
...
def _run_profile(
    ...
    *,
    ...
    if self_join:
        exclusion = (m + 3) // 4
    elif allow_trivial_match:
        exclusion = 0
    else:
        exclusion = (m + 3) // 4

    if profile == "1nn":
        return _best_match_profile(prepared_a, prepared_b, m, pearson, exclusion)
    if profile == "sum":
        return _sum_threshold_profile(prepared_a, prepared_b, threshold, exclusion)
    if profile == "matrix":
        return _matrix_summary(prepared_a, prepared_b, m, pearson, threshold, mheight, mwidth, self_join, exclusion)
    if profile == "knn":
        if k is None or k <= 0:
            raise ValueError("k must be greater than 0")
        return _knn_profile(prepared_a, prepared_b, m, k, threshold, pearson, exclusion)
...
def abjoin_sum(a: Any, b: Any, m: int, **kwargs: Any) -> np.ndarray:
    params = _parse_common_kwargs(kwargs, allow_threshold=True, is_ab_join=True)
    return _run_profile(
        a,
        b,
        m,
...
def abjoin_matrix(a: Any, b: Any, m: int, **kwargs: Any) -> np.ndarray:
    params = _parse_common_kwargs(kwargs, allow_threshold=True, allow_matrix=True, is_ab_join=True)
    return _run_profile(
        a,
        b,
        m,
