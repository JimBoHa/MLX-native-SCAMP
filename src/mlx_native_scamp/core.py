...
def _portable_best_match(
    ...
    *,
    ...
    """Reduce portable 1NN values, optionally retaining match indices."""

    best_corr = mx.full(
        (prepared_a.subsequences,), SENTINEL, dtype=prepared_a.windows.dtype
    )
    best_idx = (
        mx.full((prepared_a.subsequences,), -1, dtype=mx.int32)
        if include_indices
        else None
    )
    for row_start, _, _, block in _iterate_blocks(
        ...
        if best_idx is not None:
            ...
            _schedule_reducer_state(best_corr)
...
def _best_match_profile(
    ...
    if use_metal_kernel:
        ...
        corr_np, idx_np = _portable_best_match(
            prepared_a,
            prepared_b,
            exclusion,
...
def _best_match_values(
    ...
    if use_metal_kernel:
        ...
        corr_np, _ = _portable_best_match(
            prepared_a,
            prepared_b,
            exclusion,
