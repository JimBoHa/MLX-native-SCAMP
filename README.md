...
- `mlx_native_scamp.selfjoin_1nn` and `abjoin_1nn` expose SCAMP's index-free
  `1NN` profile for native integrations. They return only profile values and
  omit index reduction on the portable path and Metal's second index-selection
  pass. The strict `pyscamp` compatibility namespace remains limited to the
  upstream Python exports.
